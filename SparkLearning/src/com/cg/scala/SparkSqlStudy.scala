...
import com.cg.scala.udf.{AddTitleUDF, GetJsonObjectUDF}
...
//    useSparkSession(args)
  ...
  def useSqlContext(args: Array[String]): Unit = {
    ...
    val teenagers = sqlContext.sql("SELECT addTitle(name) as nameLength, age FROM people WHERE age >= 13 AND age <= 24")
