...
public class CallLogJob001 extends BaseJob{
	...
	public static class CallLogMapper extends
			NiuwaMapper<LongWritable, Text, Text, Text> {
	...
	public static class CallLogReducer extends NiuwaReducer<Text, Text, NullWritable, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text value : values) {
				...
				if (Rules.callLogBaseRule(callLog) && Rules.isMatchedRule_1(device_activation)) {
					call_sum++;
				...
				if (Rules.isMatchedRule_2_1(callLog.getLong("call_date")) && Rules
